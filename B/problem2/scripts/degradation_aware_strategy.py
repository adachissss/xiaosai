...
"""第二问第 2 小问：考虑电池寿命损耗的协同优化模型。

该模型以第一问 S3 完整协同 LP 为基础，保持约束体系不变，
在目标函数中加入固定储能和每辆 EV 的等效吞吐寿命损耗成本。
"""
...
def build_and_solve_degradation_aware(data: ProblemData) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """求解第二问 S4：考虑固定储能和 EV 寿命损耗的完整协同方案。"""
    ...
    if ev_cost_col not in ev.columns:
        raise KeyError(f"ev_sessions.csv is missing {ev_cost_col}")
    ...
    if not result.success:
        raise RuntimeError(f"考虑寿命损耗的协同优化模型求解失败：{result.message}")
    ...
    return schedule, ev_result, {
        ...
        "scheme": "S4_degradation_aware",
        "battery_unit_degradation_cost_cny_per_kwh": battery_deg_cost,
        "ev_degradation_cost_cny": float(ev_result["degradation_cost_cny"].sum()),
    }
